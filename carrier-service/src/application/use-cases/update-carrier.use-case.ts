...
export class UpdateCarrierUseCase {
  ...
  async execute(id: number, updateCarrierDto: UpdateCarrierDto): Promise<CarrierResponseDto> {
    ...
    if (updateCarrierDto.name && updateCarrierDto.name !== existingCarrier.name) {
      const carrierWithSameName = await this.carrierRepository.findByName(updateCarrierDto.name);
    ...
    if (updateCarrierDto.metadata !== undefined) {
      const metadataValidation = this.carrierDomainService.validateMetadata(updateCarrierDto.metadata);
    ...
    if (updateCarrierDto.name !== undefined) updateData.name = updateCarrierDto.name;
    if (updateCarrierDto.description !== undefined) updateData.description = updateCarrierDto.description;
    if (updateCarrierDto.isActive !== undefined) updateData.isActive = updateCarrierDto.isActive;
    if (updateCarrierDto.contactEmail !== undefined) updateData.contactEmail = updateCarrierDto.contactEmail;
    if (updateCarrierDto.contactPhone !== undefined) updateData.contactPhone = updateCarrierDto.contactPhone;
    if (updateCarrierDto.metadata !== undefined) updateData.metadata = updateCarrierDto.metadata;
