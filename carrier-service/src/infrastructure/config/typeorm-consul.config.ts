/**
 * TypeORM Configuration with Consul Integration - Carrier Service
 * 
...
import { CarrierTypeOrmEntity } from '../database/typeorm/entities/carrier.typeorm.entity';
...
export async function createTypeOrmConsulConfig(): Promise<TypeOrmModuleOptions> {
  ...
  try {
    ...
    const dbUsername = await consulConfig.get(
      'database/username',
      process.env.DB_USERNAME || 'carrier_user'
    );
    const dbPassword = await consulConfig.get(
      'database/password',
      process.env.DB_PASSWORD || 'carrier_password'
    );
    const dbName = await consulConfig.get(
      'database/database',
      process.env.DB_NAME || 'carrier_service_db'
    );
