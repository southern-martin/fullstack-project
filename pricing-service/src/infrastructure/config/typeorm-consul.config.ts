/**
 * TypeORM Configuration with Consul Integration - Pricing Service
 * 
...
import { PriceCalculationTypeOrmEntity } from '../database/typeorm/entities/price-calculation.typeorm.entity';
import { PricingRuleTypeOrmEntity } from '../database/typeorm/entities/pricing-rule.typeorm.entity';
...
export async function createTypeOrmConsulConfig(): Promise<TypeOrmModuleOptions> {
  ...
  try {
    ...
    const dbUsername = await consulConfig.get(
      'database/username',
      process.env.DB_USERNAME || 'pricing_user'
    );
    const dbPassword = await consulConfig.get(
      'database/password',
      process.env.DB_PASSWORD || 'pricing_password'
    );
    const dbName = await consulConfig.get(
      'database/database',
      process.env.DB_NAME || 'pricing_service_db'
    );
