...
/**
 * Consul Configuration Test Script - Pricing Service
 * 
...
async function testServiceConfiguration(): Promise<boolean> {
  header('Testing Pricing Service-Specific Configuration');
  ...
  try {
    ...
    if (port === 3006) {
      ...
      error(`Service port incorrect: ${port} (expected 3006)`);
      allPassed = false;
    }
    
    // Test service name
    const serviceName = await consulService.get('service_name');
    if (serviceName === 'pricing-service') {
      success(`Service name: ${serviceName}`);
    } else {
      error(`Service name incorrect: ${serviceName} (expected pricing-service)`);
      allPassed = false;
    }
    
    // Test Redis key prefix
    const keyPrefix = await consulService.get('redis_key_prefix');
    if (keyPrefix === 'pricing') {
      success(`Redis key prefix: ${keyPrefix}`);
    } else {
      error(`Redis key prefix incorrect: ${keyPrefix} (expected pricing)`);
      allPassed = false;
    }
...
async function testDatabaseConfiguration(): Promise<boolean> {
  ...
  try {
    ...
    if (dbHost === 'pricing-service-db') {
      success(`Database host: ${dbHost}`);
    } else {
      error(`Database host incorrect: ${dbHost} (expected pricing-service-db)`);
      allPassed = false;
    }
    ...
    if (dbUsername === 'pricing_user') {
      success(`Database username: ${dbUsername}`);
    } else {
      error(`Database username incorrect: ${dbUsername} (expected pricing_user)`);
      allPassed = false;
    }
    
    const dbPassword = await consulService.get('database/password');
    if (dbPassword === 'pricing_password') {
    ...
    if (dbName === 'pricing_service_db') {
      success(`Database name: ${dbName}`);
    } else {
      error(`Database name incorrect: ${dbName} (expected pricing_service_db)`);
      allPassed = false;
    }
...
async function testTypeOrmConfiguration(): Promise<boolean> {
  ...
  try {
    ...
    if (typeOrmConfig.host === 'pricing-service-db') {
      success(`TypeORM host: ${typeOrmConfig.host}`);
    } else {
      error(`TypeORM host incorrect: ${typeOrmConfig.host} (expected pricing-service-db)`);
      allPassed = false;
    }
    ...
    if (typeOrmConfig.database === 'pricing_service_db') {
      success(`TypeORM database: ${typeOrmConfig.database}`);
    } else {
      error(`TypeORM database incorrect: ${typeOrmConfig.database} (expected pricing_service_db)`);
      allPassed = false;
    }
...
async function testAllConfigurations(): Promise<void> {
  header('🧪 PRICING SERVICE - CONSUL CONFIGURATION TEST SUITE');
  info('Testing Consul integration for Pricing Service');
  ...
  try {
    ...
    if (passedTests === totalTests) {
      ...
      success('Pricing Service Consul integration is working correctly!');
      process.exit(0);
    } else {
