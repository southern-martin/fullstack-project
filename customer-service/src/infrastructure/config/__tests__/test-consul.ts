...
/**
 * Consul Configuration Test Script - Customer Service
 * 
...
async function testServiceConfiguration(): Promise<boolean> {
  header('Testing Customer Service-Specific Configuration');
  ...
  try {
    ...
    if (port === 3004) {
      ...
      error(`Service port incorrect: ${port} (expected 3004)`);
      allPassed = false;
    }
    ...
    if (serviceName === 'customer-service') {
      ...
      error(`Service name incorrect: ${serviceName} (expected customer-service)`);
      allPassed = false;
    }
    
    // Test Redis key prefix
    const keyPrefix = await consulService.get('redis_key_prefix');
    if (keyPrefix === 'customer') {
      success(`Redis key prefix: ${keyPrefix}`);
    } else {
      error(`Redis key prefix incorrect: ${keyPrefix} (expected customer)`);
      allPassed = false;
    }
...
async function testDatabaseConfiguration(): Promise<boolean> {
  ...
  try {
    ...
    if (dbHost === 'customer-service-db') {
      success(`Database host: ${dbHost}`);
    } else {
      error(`Database host incorrect: ${dbHost} (expected customer-service-db)`);
      allPassed = false;
    }
    ...
    if (dbUsername === 'customer_user') {
      success(`Database username: ${dbUsername}`);
    } else {
      error(`Database username incorrect: ${dbUsername} (expected customer_user)`);
      allPassed = false;
    }
    
    const dbPassword = await consulService.get('database/password');
    if (dbPassword === 'customer_password') {
    ...
    if (dbName === 'customer_service_db') {
      success(`Database name: ${dbName}`);
    } else {
      error(`Database name incorrect: ${dbName} (expected customer_service_db)`);
      allPassed = false;
    }
...
async function testTypeOrmConfiguration(): Promise<boolean> {
  ...
  try {
    ...
    if (typeOrmConfig.host === 'customer-service-db') {
      success(`TypeORM host: ${typeOrmConfig.host}`);
    } else {
      error(`TypeORM host incorrect: ${typeOrmConfig.host} (expected customer-service-db)`);
      allPassed = false;
    }
    ...
    if (typeOrmConfig.database === 'customer_service_db') {
      success(`TypeORM database: ${typeOrmConfig.database}`);
    } else {
      error(`TypeORM database incorrect: ${typeOrmConfig.database} (expected customer_service_db)`);
      allPassed = false;
    }
...
async function testAllConfigurations(): Promise<void> {
  header('🧪 CUSTOMER SERVICE - CONSUL CONFIGURATION TEST SUITE');
  info('Testing Consul integration for Customer Service');
  ...
  try {
    ...
    if (passedTests === totalTests) {
      ...
      success('Customer Service Consul integration is working correctly!');
      process.exit(0);
    } else {
