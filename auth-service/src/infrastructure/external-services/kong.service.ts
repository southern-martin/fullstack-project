...
export class KongService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    // Validate required Kong configuration
    this.kongAdminUrl = this.configService.get<string>('KONG_ADMIN_URL');
    if (!this.kongAdminUrl) {
      throw new Error('KONG_ADMIN_URL environment variable is required');
    }

    const kongAdminToken = this.configService.get<string>('KONG_ADMIN_TOKEN');
    this.enabled = this.configService.get<string>('KONG_SYNC_ENABLED') === 'true';
    ...
    if (!this.enabled) {
      ...
      this.logger.log(`Kong service initialized with admin URL: ${this.kongAdminUrl}`);
    }
  }
   ...
   */
  async createKongConsumer(userId: number, username: string, roles: Role[]): Promise<void> {
    if (!this.enabled) {
    ...
    try {
      ...
      // 1. Create consumer
      const consumerResponse = await this.client.post<KongConsumer>('/consumers', {
      ...
      this.logger.log(`Kong consumer created: ${username} (ID: ${consumerId})`, {
        userId,
        consumerId,
      });
      ...
      try {
        ...
        this.logger.error(`Failed to sync JWT credential for consumer ${consumerId}: ${error}`);
      }
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (error.response?.status === 409) {
          this.logger.warn(`Kong consumer already exists: ${username}, updating instead`, {
            userId,
          });
          await this.updateKongConsumerGroups(userId, roles);
        } else {
          this.logger.error(`Failed to create Kong consumer: ${error.message}`, error.stack);
        }
      } else {
        this.logger.error(
   ...
   */
  async updateKongConsumerGroups(userId: number, roles: Role[]): Promise<void> {
    if (!this.enabled) {
    ...
    try {
      ...
      if (!consumer) {
        this.logger.warn(`Kong consumer not found for user ${userId}, cannot update ACL groups`);
        return;
      }
      ...
      for (const roleName of roleNames) {
        ...
        this.logger.debug(`ACL group '${roleName}' added to consumer ${consumer.username}`, {
          userId,
      ...
      if (axios.isAxiosError(error)) {
        this.logger.error(`Failed to update Kong ACL groups: ${error.message}`, error.stack);
      } else {
        this.logger.error(
  ...
  async deleteKongConsumer(userId: number): Promise<void> {
    ...
    try {
      ...
      const consumer = await this.getConsumerByCustomId(customId);
      if (!consumer) {
        this.logger.warn(`Kong consumer not found for user ${userId}, nothing to delete`);
        return;
      }
      ...
      if (axios.isAxiosError(error)) {
        this.logger.error(`Failed to delete Kong consumer: ${error.message}`, error.stack);
      } else {
        this.logger.error(
  ...
  async syncKongConsumerJWT(userId: number, username: string): Promise<void> {
    ...
    try {
      ...
      const consumer = await this.getConsumerByCustomId(customId);
      if (!consumer) {
        this.logger.warn(`Kong consumer not found for user ${userId}, cannot sync JWT`);
        return;
      }

      // Check if JWT credential already exists
      const existingJwts = await this.client.get(`/consumers/${consumer.id}/jwt`);
      if (existingJwts.data.data && existingJwts.data.data.length > 0) {
        this.logger.debug(`JWT credential already exists for consumer ${consumer.username}`, {
          userId,
          consumerId: consumer.id,
        });
        return;
      ...
      await this.client.post<KongJwtCredential>(`/consumers/${consumer.id}/jwt`, {
      ...
      if (axios.isAxiosError(error)) {
        this.logger.error(`Failed to sync JWT credential: ${error.message}`, error.stack);
      } else {
        this.logger.error(
   ...
   */
  private async getConsumerByCustomId(customId: string): Promise<KongConsumer | null> {
    try {
      const response = await this.client.get<KongConsumer>(`/consumers/${customId}`);
