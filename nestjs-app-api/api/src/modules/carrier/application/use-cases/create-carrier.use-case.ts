...
export class CreateCarrierUseCase {
  ...
  async execute(createCarrierDto: CreateCarrierDto): Promise<CarrierResponseDto> {
    ...
    if (createCarrierDto.metadata?.code) {
      const existingCarrier = await this.carrierRepository.findByCode(createCarrierDto.metadata.code);
      if (existingCarrier) {
        throw new ConflictException(`Carrier with code ${createCarrierDto.metadata.code} already exists`);
      }
    }

    // 3. Normalize carrier data
    const normalizedData = this.carrierDomainService.normalizeCarrierData(createCarrierDto);
