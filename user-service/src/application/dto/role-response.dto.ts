...
export class RoleResponseDto {
  ...
  @ApiProperty({ description: 'Array of permission names', example: ['users.read', 'users.write'], type: [String] })
  permissions: string[]; // Array of permission names from relational system (permissionEntities)
  
  @ApiPropertyOptional({ description: 'Detailed permission objects', type: [PermissionResponseDto] })
  permissionDetails?: PermissionResponseDto[]; // Detailed permission objects with categories
  
  @ApiPropertyOptional({ description: 'Count of permissions assigned to this role', example: 5 })
  permissionsCount?: number; // Count of permissions assigned to this role
  
  @ApiPropertyOptional({ description: 'Count of users with this role', example: 10 })
