...
async function seed400Users() {
  ...
  try {
    ...
    for (let i = 0; i < totalUsers; i += batchSize) {
      ...
      console.log(
        `📊 Progress: ${progress}/${totalUsers} (${percentage}%) - ✅ ${successCount} success, ❌ ${errorCount} failed - ⏱️  ${elapsed}s`
      );
    }
    ...
    console.log(
      `   ⚡ Average: ${(totalUsers / parseFloat(totalTime)).toFixed(2)} users/second`
    );
    console.log("=".repeat(60));
    console.log(`\n💡 Tips:`);
